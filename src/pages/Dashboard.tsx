...
export function Dashboard() {
	...
	const cpuSub = system.data
		? `${system.data.cpus} ליבות · ${cpuStatusLabel(load)}`
		: undefined;
			...
			<div className="p-4 space-y-4" dir="rtl">
				...
				{system.data?.memory && (
					<StatusCard
						title="זיכרון"
						value={formatMemory(
							system.data.memory.used,
							system.data.memory.total,
						)}
						icon="💾"
						subtitle={
							system.data.memory.usePct
								? `${system.data.memory.usePct} בשימוש`
								: undefined
						}
				...
				<section className="animate-card [animation-delay:260ms]">
					<h2 className="text-base font-semibold mb-2">
						משימות מתוזמנות ({enabledCrons.length})
					</h2>
					<div className="rounded-xl bg-tg-section-bg px-4">
						{enabledCrons.length === 0 ? (
							<p className="py-4 text-center text-tg-hint text-sm">
								אין משימות פעילות
							</p>
						) : (
