...
export function Expenses() {
  const [expenses, setExpenses] = useState<Expense[]>(MOCK);
  ...
  const byCategory = CATEGORIES.map((cat) => ({
    ...cat,
    sum: expenses.filter((e) => e.category === cat.id).reduce((s, e) => s + e.amount, 0),
  }));
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {/* Category breakdown */}
      <section>
        <h2 className="text-sm font-semibold text-tg-section-header mb-2">פירוט לפי קטגוריה</h2>
      ...
      <section className="rounded-xl bg-tg-section-bg p-4 space-y-3">
        <h2 className="text-sm font-semibold text-tg-section-header">הוסף הוצאה</h2>
      ...
      {/* Recent transactions */}
      <section>
        <h2 className="text-sm font-semibold text-tg-section-header mb-2">עסקאות אחרונות</h2>
        <div className="rounded-xl bg-tg-section-bg divide-y divide-tg-secondary-bg">
          {expenses.map((e) => {
